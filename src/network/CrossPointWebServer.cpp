...
void CrossPointWebServer::begin() {
  ...
  server->on("/api/apps", HTTP_GET, [this] { handleAppList(); });
  server->on("/api/apps/launch", HTTP_POST, [this] { handleLaunchApp(); });
  server->on("/api/reader/open", HTTP_POST, [this] { handleOpenReader(); });
    ...
    if (packetSize > 0) {
      ...
      if (len > 0) {
        ...
        if (strcmp(buffer, "hello") == 0) {
          ...
          String message = "omnipaper (on " + hostname + ");" + String(wsPort);
...
void CrossPointWebServer::handleStatus() const {
  ...
  doc["appLaunchAvailable"] = static_cast<bool>(appLaunchCallback);
  doc["readerOpenAvailable"] = static_cast<bool>(readerOpenCallback);
...
void CrossPointWebServer::handleAppList() const {
  JsonDocument doc;
  JsonArray apps = doc["apps"].to<JsonArray>();
  for (size_t i = 0; i < WEB_UI_APPS_COUNT; i++) {
    JsonObject app = apps.add<JsonObject>();
    app["id"] = WEB_UI_APPS[i].id;
    app["name"] = WEB_UI_APPS[i].name;
  }
  doc["count"] = WEB_UI_APPS_COUNT;
...
void CrossPointWebServer::handleLaunchApp() const {
  if (!server->hasArg("id")) {
    server->send(400, "text/plain", "Missing app id");
    return;
  }

  if (!appLaunchCallback) {
    server->send(503, "text/plain", "App launch not available");
    return;
  }

  const String appId = server->arg("id");
  if (appId.isEmpty()) {
    server->send(400, "text/plain", "Invalid app id");
    return;
  }

  const bool accepted = appLaunchCallback(appId.c_str());
  if (!accepted) {
    server->send(409, "text/plain", "App launch busy or unsupported");
    return;
  }

  server->send(202, "text/plain", "Launch queued: " + appId);
}

void CrossPointWebServer::handleOpenReader() const {
  if (!server->hasArg("path")) {
    server->send(400, "text/plain", "Missing path");
    return;
  }

  if (!readerOpenCallback) {
    server->send(503, "text/plain", "Reader open not available");
    return;
  }
  ...
  if (!StringUtils::checkFileExtension(path, ".epub")) {
    server->send(400, "text/plain", "Path must be an .epub file");
    return;
  }
  ...
  const bool accepted = readerOpenCallback(path.c_str());
  if (!accepted) {
    server->send(409, "text/plain", "Reader open busy");
    return;
  }

  server->send(202, "text/plain", "Reader open queued: " + path);
}
