...
constexpr int PREVIEW_PANEL_WIDTH = 180;
constexpr int PREVIEW_IMAGE_WIDTH = 140;
constexpr int PREVIEW_IMAGE_HEIGHT = 200;
constexpr int PREVIEW_MIN_LIST_WIDTH = 180;
...
void MyLibraryActivity::invalidateFilePreview() {
  filePreviewKey.clear();
  filePreviewBmpPath.clear();
  filePreviewTitle.clear();
  filePreviewAuthor.clear();
  filePreviewResolved = false;
}

void MyLibraryActivity::prepareFilePreview() {
  const std::string selectedPath = buildSelectedFilePath();
  if (selectedPath.empty()) {
    invalidateFilePreview();
    return;
  }

  if (filePreviewResolved && selectedPath == filePreviewKey) {
    return;
  }

  std::string previewPath;
  std::string previewTitle;
  std::string previewAuthor;

  std::string filename = selectedPath;
  ...
  if (StringUtils::checkFileExtension(selectedPath, ".epub")) {
    Epub epub(selectedPath, "/.crosspoint");
    if (epub.load(false)) {
      if (!epub.getTitle().empty()) {
        previewTitle = epub.getTitle();
      }
      if (!epub.getAuthor().empty()) {
        previewAuthor = epub.getAuthor();
      }
      if (epub.generateThumbBmp()) {
        previewPath = epub.getThumbBmpPath();
      } else if (epub.generateCoverBmp(false)) {
        previewPath = epub.getCoverBmpPath(false);
      }
    }
  } else if (StringUtils::checkFileExtension(selectedPath, ".xtc") ||
             StringUtils::checkFileExtension(selectedPath, ".xtch")) {
    Xtc xtc(selectedPath, "/.crosspoint");
    if (xtc.load()) {
      ...
      if (xtc.generateThumbBmp()) {
        previewPath = xtc.getThumbBmpPath();
      } else if (xtc.generateCoverBmp()) {
    ...
    if (txt.load()) {
      ...
      if (txt.generateCoverBmp()) {
        previewPath = txt.getCoverBmpPath();
      }
    }
  }

  if (!previewPath.empty() && !SdMan.exists(previewPath.c_str())) {
    previewPath.clear();
  }

  filePreviewKey = selectedPath;
  filePreviewBmpPath = previewPath;
  filePreviewTitle = previewTitle;
  filePreviewAuthor = previewAuthor;
  filePreviewResolved = true;
}
...
void MyLibraryActivity::renderFilesTab() const {
  ...
  const bool showPreview = pageWidth > (PREVIEW_MIN_LIST_WIDTH + PREVIEW_PANEL_WIDTH + LEFT_MARGIN + RIGHT_MARGIN);
  const int listWidth = showPreview ? pageWidth - PREVIEW_PANEL_WIDTH - RIGHT_MARGIN : pageWidth - RIGHT_MARGIN;
  const int itemMaxWidth = listWidth - LEFT_MARGIN - 8;
  ...
  // Draw selection highlight
  renderer.fillRect(0, CONTENT_START_Y + (selectorIndex % pageItems) * LINE_HEIGHT - 2, listWidth, LINE_HEIGHT);

  // Draw items
  for (int i = pageStartIndex; i < fileCount && i < pageStartIndex + pageItems; i++) {
    auto item = renderer.truncatedText(UI_10_FONT_ID, files[i].c_str(), itemMaxWidth);
  ...
  const int panelX = pageWidth - PREVIEW_PANEL_WIDTH + 4;
  const int panelW = PREVIEW_PANEL_WIDTH - 12;
  const int imageX = panelX + (panelW - PREVIEW_IMAGE_WIDTH) / 2;
  const int imageY = CONTENT_START_Y + 4;

  renderer.drawRect(panelX, CONTENT_START_Y - 4, panelW, renderer.getScreenHeight() - CONTENT_START_Y - 56);
  renderer.drawRect(imageX, imageY, PREVIEW_IMAGE_WIDTH, PREVIEW_IMAGE_HEIGHT);

  bool drewPreview = false;
  if (!filePreviewBmpPath.empty()) {
    FsFile previewFile;
    if (SdMan.openFileForRead("MLB", filePreviewBmpPath, previewFile)) {
      Bitmap bitmap(previewFile);
      if (bitmap.parseHeaders() == BmpReaderError::Ok) {
        renderer.drawBitmap(bitmap, imageX, imageY, PREVIEW_IMAGE_WIDTH, PREVIEW_IMAGE_HEIGHT);
        drewPreview = true;
      }
      previewFile.close();
    }
  ...
  if (!drewPreview) {
    renderer.drawCenteredText(UI_10_FONT_ID, imageY + PREVIEW_IMAGE_HEIGHT / 2, "No cover");
  }

  const int textWidth = panelW - 12;
  int textY = imageY + PREVIEW_IMAGE_HEIGHT + 14;
  const std::string title = filePreviewTitle.empty() ? files[selectorIndex] : filePreviewTitle;
  const auto titleLine = renderer.truncatedText(UI_10_FONT_ID, title.c_str(), textWidth);
  renderer.drawText(UI_10_FONT_ID, panelX + 6, textY, titleLine.c_str());
  textY += 20;

  if (!filePreviewAuthor.empty()) {
    const auto authorLine = renderer.truncatedText(SMALL_FONT_ID, filePreviewAuthor.c_str(), textWidth);
    renderer.drawText(SMALL_FONT_ID, panelX + 6, textY, authorLine.c_str());
  }
}
