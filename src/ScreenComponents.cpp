...
std::string buildDateTimeText() {
  ...
  char dateTime[24];
  snprintf(dateTime, sizeof(dateTime), "%04d/%02d/%02d %02d:%02d", localTime.tm_year + 1900, localTime.tm_mon + 1,
           localTime.tm_mday, localTime.tm_hour, localTime.tm_min);
  return dateTime;
}

std::string buildWifiText() {
  if (WiFi.status() != WL_CONNECTED) {
    return "WiFi:off";
  }

  char wifi[24];
  snprintf(wifi, sizeof(wifi), "WiFi:%ddBm", WiFi.RSSI());
  return wifi;
}
}  // namespace
...
void ScreenComponents::drawDeviceInfoOverlay(const GfxRenderer& renderer, const uint8_t overlayPosition) {
  if (overlayPosition == CrossPointSettings::OVERLAY_OFF) {
    return;
  }
  ...
  const bool isCorners = overlayPosition == CrossPointSettings::OVERLAY_TOP_CORNERS ||
                         overlayPosition == CrossPointSettings::OVERLAY_BOTTOM_CORNERS;
  const int y = isBottom ? bottomY : topY;
